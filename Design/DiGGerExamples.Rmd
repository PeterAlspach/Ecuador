...
We have access to a number of trial design software packages (e.g., CycDesigN and the [dae](https://cran.stat.auckland.ac.nz/web/packages/dae/index.html) R package).  I find [`DiGGer`](http://www.austatgen.org/software/) particularly good for breeding trials.  It is available both as a stand-alone executable and as an R-package.  Herein we discuss `r citation('DiGGer')[[1]]$note`.

`DiGGer` is capable of a wide range of designs, some of which are discussed in the ['Quick Guide'](https://iplant.plantandfood.co.nz/biometrics/Documents/DiGGerQuickGuide.pdf) for the R package which comes with the software (but unfortunately not as a vignette).  In particular, the initial swap matrix (see the 'Dummy example' in the 'Seedlings' section below for a simple example) gives `DiGGer` enormous flexibility to deal with non-rectangular blocks, gaps due to existing plants or other structures, alternative blocking criteria (e.g., one might wish to block an experiment on exsiting trees according to some characterisitics of those trees, such as size or disease status, rather than location) and so on.  The text herein is not intended to replicate the 'Quick Guide', but rather to illustrate how `DiGGer` can be used for some of the common situations that arise in the PFR fruit breeding programmes.

The trial should first be set-up in E-Brida, which will define the number of plots and the genotypes.  **After `DiGGer` has created the design, guard plants (and perhaps rows) should be added**.  I create an Excel spreadsheet with the field plan, and ensure the code used to create it is on the sheet (e.g., [BlueberrySeedling2015Design.xlsx](https://iplant.plantandfood.co.nz/Team/bg/premcrop/Data(Research)/BlueberrySeedling2015Design.xlsx)).  Since there is generally a bit of manual manipulation involved in creating this plan, I finish with a sanity check involving reading the plan back into R and checking that the numbers match.

## Preliminaries

It is assumed that the software has been downloaded from the [austatgen site](http://www.austatgen.org/software/), unzipped and the package installed.  The package depends on `R.oo`, `R.methodsS3` and `MASS` - all of which can be installed from the nearest [CRAN mirror](http://cran.stat.auckland.ac.nz/).
...
* The code to plot the design perhaps requires a little explaining
    ...
    + Alternatively, one can use `PAAUtils::plotDesign(getDesign(desDiG), 'red')` which cycles through the treatments high-lighting each one in turn (press \<Esc\> to move to the next one).
...
<!-- Since `r2dDiGGer` is a wrapper for `corDiGGer` which generates an initial design and swap matrices, one could probably get around this by 'manually' creating these initial matrices and calling `corDiGGer` directly.  However, I tend to use version 0.2-31 instead. -->

<!-- ```{r rowColDesign0.2-31} -->
<!-- detach("package:DiGGer", unload=TRUE) -->
<!-- library(DiGGer, lib.loc='K:/PAA/R/R-3.3.0/library') # specify location of 0.2-31 -->

<!-- trtNames <- paste0('Sel', formatC(1:12, flag=0, width=2)) -->
<!-- nTrts <- length(trtNames) -->
<!-- nReps <- 3 -->
<!-- # Create design -->
<!-- tmpDiG <- DiGGer(NumberOfTreatments=nTrts, RowsInDesign=nTrts/2, ColumnsInDesign=nReps*2, -->
<!--                  RowColumn=TRUE, BlockIn2D=TRUE, TreatmentType='FIXED', -->
<!--                  TreatmentName=trtNames, RNGSeeds=NULL) -->
<!-- desDiG <- run(tmpDiG) -->
<!-- planDiG <- matrix(desDiG$dlist$ENTRY, nrow=nTrts/2) -->
<!-- detach("package:DiGGer", unload=TRUE) -->

<!-- library(DiGGer) # using version 1.0.1 -->
<!-- # Plot design created using 0.2-31 using desPlot from 1.0.1 -->
<!-- ttMat <- matrix(rep(1:nReps, each=nTrts), ncol=nReps*2, byrow=TRUE) -->
<!-- desPlot(ttMat, trts=1, label=FALSE) -->
<!-- desPlot(ttMat, trts=2, label=FALSE, col='grey70', new=FALSE) -->
<!-- desPlot(ttMat, trts=3, label=FALSE, new=FALSE) -->
<!-- desPlot(planDiG, label=TRUE, bdef=cbind(nTrts/2,2), bcol='red4', new=FALSE, ) -->
<!-- ``` -->


#### Incomplete block design

In this case, imagine a trial to assess 66 advanced selections with two replicates of each, with two-tree plots.  Something like this is not uncommon in the apple programme for example.  One could do a row-column design as above, but the blocks are likely to be too big to consider them 'uniform' in any meaningful sense.  Thus an incomplete block design with 11 plots (22 trees) per incomplete block might be better.
...
* `getConcurrence` shows the number of pairs of treatments which don't occur together in any incomplete block (`r concur[1]`), those that occur in only one incomplete block (`r concur[2]`) and those that occur in two incomplete blocks (`r concur[3]`).  These should add up to (66^2^-66)/2 = `r (66^2-66)/2`, which it does
...
For seedling trials there are typically a number of families (maybe 10-50) and the number of genotypes per family can vary by a couple of orders of magnitude.  But the genotypes themselves are rarely replicated.  Some breeders use check cultivars in their trials and others don't.  Although `DiGGer` allows for placement of checks using a different randomisation strategy, this is not particularly helpful with most PFR perennial breeding trials.  This is because the seedlings are usually planted in family plots (mostly defined by the planting system; e.g., seven-plant bays in kiwifruit), whereas the checks are planted as individual plants.
...
* The first section (four lines) attempts to create a dataset as might have been retrieved from E-Brida.  It will not always look like this.  For example, if the plants have not been individualised (i.e., a nursery trial) the pedigree item names will just be the family and the checks are unlikely to be present
* In the set-up phase
...
* Assign GroupCodes (i.e., the plant location codes as used by the particular crop)
...
_Left over plants_  
I generally leave these for the breeder to deal with.  Sometimes they simply discard them, or keep them for replants; other times they fill in the ends of rows^[Pipfruit breeders tend to plant by machine and thus often have ragged rows to complete.]
...
The initial swap matrix `initSwap` can have values other than 0 and 1.  For example, in the 'Motueka 2016 Block R3 Seedlings' trial (in E-Brida, with the [DiGGer code on github](https://github.com/PlantandFoodResearch/Biometrics/blob/PeterAlspach/BreedingAnalyses/Kiwifruit/Novels/Motueka2016BlockR3SeedlingsDiGGerCode.R)), bays 1, 4, 9 (and 14 but this was not relevant for the first planting) of rows 501, 505, 509, 513, 517 and 521 had a post for holding the shelter and thus room for only 6 rather than 7 plants.  One approach would be to ignore this at the design stage, and simply omit the plant as appropriate and add it to the list of extras.  Another approach (used in the code) is to utilise these smaller plots to accommodate the families with 4, 5 or 6 left over plants.  This was achieved by giving these bays initial swap values of 2, and setting up the initial design to ensure the desired families went there.
